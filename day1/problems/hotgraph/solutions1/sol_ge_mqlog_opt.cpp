...
int bfs(int start, int finish, int diff) {
	...
	for (int i = 0; i < len[start]; i++) {
   		...
   		myqueue[rbq++] = edgenum[start][i] ^ 1;
   		if (numv[edgenum[start][i] ^ 1] == finish)
   			return 1;
   	...
   	while (lbq < rbq) {
   		...
   		for (int i = 0; i < 2; i++) {
   			while (true) {
   				...
   				if (numv[edgenum[myv][nrs] ^ 1] == finish)
   					return dist[edge] + 1;

   				dist[edgenum[myv][nrs] ^ 1] = dist[edge] + 1;
   				myqueue[rbq++] = edgenum[myv][nrs] ^ 1;
   				left[myv][nrs] = nrs - 1;
   				right[myv][nrs] = nrs + 1;
   				if (nrs + 1 == len[myv])
   					right[myv][nrs] = -1;
