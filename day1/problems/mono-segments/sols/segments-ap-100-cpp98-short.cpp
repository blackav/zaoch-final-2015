...
size_t computeSegmentsCount(vector<short> cell_colors) {
...
size_t computeNeighborhoodSegmentsCount(BidirectionalIterator begin, BidirectionalIterator base, BidirectionalIterator end) {
    ...
    return
        1
        + (base != begin && *prev != *base)
        + (next != end && *next != *base);
}


vector<size_t> processCellRecolorQueries(vector<short> cell_colors, const vector<CellRecolorQuery> &cell_recolor_queries) {
...
vector<short> readCellColors(std::istream &in_stream = std::cin) {
    size_t cells_count;
    in_stream >> cells_count;

    vector<short> cell_colors;
...
int main()
{
    ...
    vector<short> cell_colors = readCellColors(in_stream);
