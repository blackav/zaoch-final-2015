...
bool jump(int& x, const int len, const ll val) { 
  if ((h[x] - val - h[x + len] * stp[len]) % md == 0) {
    int k = len * 2;
    while ((h[x] - h[x + len] + (h[x + k] - h[x + k - len]) * stp[k - len]) % md == 0) {
      k *= 2;
    }
    int l = k / 2;
    int r = k;
    while (r - l > len) {
      int m = (l + r) / 2;
      if ((h[x] - h[x + len] + (h[x + m] - h[x + m - len]) * stp[m - len]) % md == 0) {
        l = m;
      } else {
        r = m;
      }
...
void calc_2() {
  ...
  for (int l1 = 1; l1 <= L; l1++) {
    int pos = 0;
    ll val1 = (h[0] - h[l1] * stp[l1]) % md;
    if (val1 < 0) {
      val1 += md;
    }
    jump(pos, l1, val1);
    for (int l2 = 1; l2 <= min(L, n - pos); l2++) if (good(n - pos, l1, l2)) {
      ll val2 = (h[pos] - h[pos + l2] * stp[l2]) % md;      
      if (val2 < 0) {
        val2 += md;
      }
      int pos2 = pos;
