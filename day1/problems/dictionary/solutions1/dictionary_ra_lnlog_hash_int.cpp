...
int C = 0;
bool jump(int& x, const int len, const int val) { 
  bool res = false;
  while (h[x] - val - h[x + len] * stp[len] == 0) {
    C++;
    if (C == 1000) {
...
/*  for (int i = 1; i <= L; i++) {
  ...
  for (int i = n - 1; i >= 0; i--) {
    h[i] = h[i + 1] * stp[1] + s[i] - 'a' + 1;
  }
  for (int l1 = 1; l1 <= L; l1++) {
    int pos = 0;
    int val1 = h[0] - h[l1] * stp[l1];
    jump(pos, l1, val1);
    for (int l2 = 1; l2 <= min(L, n - pos); l2++) if (good(n - pos, l1, l2)) {
      int val2 = h[pos] - h[pos + l2] * stp[l2];      
      int pos2 = pos;
      while (1) {
